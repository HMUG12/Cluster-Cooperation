...
1. **M2 编排收尾**：只剩轮次调度与上下文压缩策略。结构化 briefing、依赖自动解锁、评审回路、
   用量记账与预算执行都已完成并验证。
2. **M3 第二期：圆桌工具**。设计已完成（§11.14），缺的那一环（放行时按声明指派）也已补上。
   下一步是**一次调用完成"扇出 → 汇总"**：为每位参与者建一个作答任务、建一个被它们阻塞的
   汇总任务（带 `cluster-owner:` 声明）、并把问题广播出去。之后按 §11.14 的编码做**投票**
   （每票一行 + 机器可读标记 + `Tally:` 任务），最后才是**辩论**（需要轮次状态）。
   另有：把 `broadcast_message` 提升为 `tool-*` 包以进入工具目录；
