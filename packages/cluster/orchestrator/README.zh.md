...
Lead 还多了三个工具。`broadcast_message` 把一条 durable 消息发给当前每个 teammate，或发给它点名的成员，并把每一次拒绝连同原因报告回来，而不是悄悄丢掉。`roundtable` 走得更远：一次调用就把同一个问题变成若干"由成员自己拥有的作答任务"，并留下一个被全部答案阻塞的汇总任务——于是它指定的收集者会在最后一份答案落地的瞬间被指派、被唤醒。`motion` 把一个决定交给投票：每位投票人一个选票任务，外加一个 `Tally:` 任务——最后一张票投出时它被指派给某位队友，并**同时携带已经数好的结果**，因为任务板本来就知道。
...
| [`src/motion.ts`](src/motion.ts) | 投票规划、选票计数，以及那条携带计数的通知 |
...
- **仍在 provisioning 的 teammate 无法被触达** — 邮箱与任务板都把目标解析到花名册的 active 阶段，所以向一个还在启动中的成员发消息或派任务都会抛错。三种协议都把它记为 skipped 并附上原因，而不是先改动任务板、再半途失败——这正是"刚 `spawn_teammate` 完就发起扇出"会踩到的情形。
