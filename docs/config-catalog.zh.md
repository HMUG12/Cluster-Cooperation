...
来源：[`packages/cluster/orchestrator/src/index.ts:51`](../packages/cluster/orchestrator/src/index.ts)
...
- `@deepseek-ai/dsh-command-cluster` — 需要 `commands` · `agentTeams`（[`packages/cluster/command-cluster/src/index.ts`](../packages/cluster/command-cluster/src/index.ts)）
