...
| [cluster.md](cluster.md) | the cluster layer over Agent Teams: one `cluster.yml` document, the `ctx.clusterConfig` service, per-member model binding at teammate creation, and the board policy applied to completions |
